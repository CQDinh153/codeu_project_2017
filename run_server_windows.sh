...
if [[ "$TEAM_ID" == "" || "$TEAM_SECRET" == "" || "$PORT" == "" || "$PERSISTENT_DB" == "" ]] ; then
  ...
  echo 'PERSISTENT DIR : The path to the SQLite database file where the server'
  echo '                 can save data that will exist between runs.'
